...
const configureServer = () => {
  ...
  serverWebpackConfig.output = {
    ...
    globalObject: 'this',
    // Required for the React on Rails Pro node renderer.
  ...
  // The React on Rails Pro node renderer runs the server bundle in Node. Target
  // 'node' makes webpack add the 'node' resolve condition, so `react-dom/server`
  // resolves to the Node build (react-dom/server.node) instead of the browser
  // build — the browser build needs `MessageChannel`, which the renderer VM does
  // not expose (React 19). It also lets loadable-components/Emotion SSR work.
