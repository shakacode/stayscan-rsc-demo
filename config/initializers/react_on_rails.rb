...
# All frontend code lives in client/ with its own package.json/node_modules.
# Point the Shakapacker instance at that directory; the config file stays at the
# Rails root so Ruby and the webpack build read the same shakapacker.yml.
Shakapacker.instance = Shakapacker::Instance.new(
  root_path: Rails.root.join("client"),
...
ReactOnRails.configure do |config|
  ...
  #
  # React on Rails runs this from node_modules_location (client/); the shakapacker
  # binstubs live at the repo root, so step back up before invoking them.
  config.build_test_command = "cd .. && RAILS_ENV=test NODE_ENV=test bin/shakapacker-precompile-hook && SHAKAPACKER_SKIP_PRECOMPILE_HOOK=true RAILS_ENV=test NODE_ENV=test bin/shakapacker"

  config.auto_load_bundle = true
  config.components_subdirectory = "ror_components"

  # Frontend package manager + node_modules live under client/.
...
# Shared values injected into every component's railsContext.
# Assigned after initialization so the autoloaded RenderingExtension constant is
# available (referencing app/ constants inside the configure block runs too early
# in boot). React on Rails reads rendering_extension at render time.
Rails.application.config.after_initialize do
  ReactOnRails.configuration.rendering_extension = RenderingExtension
end
