...
- **Fictional brands only.** Every provider in this app is invented
  (StayScan / Airhive / Vacario / Lodgeo / Hostflow / Rentora). Do not introduce real
  booking-brand identifiers, real listing data, real photos, or secrets — everything the app
  serves is synthetic and generated.
- **Keep the complexity.** This app exists to be large enough that real RSC problems surface.
  Do not simplify a subsystem (the availability/pricing engine, the provider normalizers, the
  Redux browse page, the booking state machine) just to make a change easier.
- **Test-first for behavior changes.** Frontend changes need a system spec; system specs run in
  both desktop and mobile viewports.
- **Run the checks before committing:** `bundle exec rubocop`, `yarn --cwd client lint`,
  `bundle exec rspec`, `yarn --cwd client test`, and a `bin/shakapacker` build.
